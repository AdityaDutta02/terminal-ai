...
export async function GET(request: NextRequest) {
  ...
  try {
    const lockKey = 1000  // arbitrary unique integer for this job
    const lockResult = await db.query<{ pg_try_advisory_lock: boolean }>(
      'SELECT pg_try_advisory_lock($1)',
      [lockKey],
    )
    if (!lockResult.rows[0].pg_try_advisory_lock) {
      logger.warn({ msg: 'cron_subscription_credits_already_running', runId })
    ...
    try {
      ...
      for (const sub of subscriptions.rows) {
        await withTransaction(async (client) => {
          await grantCredits(sub.user_id, sub.credits_per_month, `subscription_renewal_${sub.plan_id}`, client)
          await client.query(
